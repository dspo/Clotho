...
12. provider 配置获取通过显式 `ConfigProvider` 边界接入；如果宿主不提供，则 framework 默认回退到读取 `~/.codex/config.toml` 的实现。
...
- Rust (示例签名，具体类型可适度调整)：
  ...
  - pub struct AgentRuntimePluginBuilder { /* config_provider(...), agent_runtime(...), include_builtin_native_tools(...)/disable_builtin_native_tools() */ }
