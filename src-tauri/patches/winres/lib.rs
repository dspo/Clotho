...
fn current_package_has_binaries() -> bool {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("."));
    let manifest = manifest_dir.join("Cargo.toml");
    let has_declared_bins = fs::read_to_string(&manifest)
        .ok()
        .and_then(|contents| contents.parse::<toml::Value>().ok())
        .and_then(|value| value.as_table().cloned())
        .map(|table| table.contains_key("bin"))
        .unwrap_or(false);

    has_declared_bins
        || manifest_dir.join("src/main.rs").exists()
        || manifest_dir.join("src/bin").is_dir()
}

fn emit_resource_linking(output_file: &Path, output_dir: &str, link_kind: &str, lib_name: &str) {
    if current_package_has_binaries() {
        // Resource outputs belong on this package's binaries; linking them as a library
        // leaks VERSIONINFO into downstream apps and causes duplicate-resource failures.
...
impl WindowsResource {
    ...
    fn compile_with_toolkit_gnu<'a>(&self, input: &'a str, output_dir: &'a str) -> io::Result<()> {
        ...
        emit_resource_linking(&libname, output_dir, "static", "resource");
    ...
    fn compile_with_toolkit_msvc<'a>(&self, input: &'a str, output_dir: &'a str) -> io::Result<()> {
        ...
        emit_resource_linking(&output, output_dir, "dylib", "resource");
        Ok(())
    }
}
